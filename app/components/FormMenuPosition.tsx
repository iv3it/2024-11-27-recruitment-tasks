...
type FormMenuPositionProps = {
  onSubmit: SubmitHandler<MenuItemType>;
  onCancel: () => void;
  initialValues?: MenuItemType | null;
};

export default function FormMenuPosition({ onSubmit, onCancel, initialValues } : FormMenuPositionProps) {
  const { register, handleSubmit, formState: { errors } } = useForm<MenuItemType>({
    defaultValues: initialValues || { id: '', name: '', url: '', children: [] },
  });

  const handleFormSubmit: SubmitHandler<MenuItemType> = (data) => {
    onSubmit(data);
  };
    ...
    <section className="flex gap-4 border border-[var(--greyLight)] rounded-lg bg-white py-5 px-6">
      <form onSubmit={handleSubmit(handleFormSubmit)} className="w-full">
        ...
        <div className="mt-5 flex gap-2">
          <button onClick={onCancel} className="shadow-[0_1px_2px_0px_rgba(16,24,40,0.05)] rounded-lg border border-[var(--greyLight)] py-[0.625rem] px-[0.875rem] font-semibold text-sm text-[var(--greyDark)]">Anuluj</button>
          <input type="submit" className="shadow-[0_1px_2px_0px_rgba(16,24,40,0.05)] rounded-lg border border-[var(--purpleLight)] py-[0.625rem] px-[0.875rem] font-semibold text-sm text-[var(--purpleDark)] cursor-pointer" value="Dodaj"/>
        </div>
      </form>
